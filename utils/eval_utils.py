...
def nms_func(p, candi_thre=0.3, xo=None, yo=None, zo=None):
    with torch.no_grad():
        ...
        conv = func.conv2d(p[:, None], gpu(filt).to('cuda:2'), padding=1)
        ...
        z_mid = zo * p
        z_conv1 = func.conv2d((z_mid * (1 - max_mask2[:, 0]))[:, None], gpu(filt).to('cuda:2'), padding=1)
        z_conv2 = func.conv2d((z_mid * (1 - max_mask1[:, 0]))[:, None], gpu(filt).to('cuda:2'), padding=1)
        ...
        xm_conv1 = func.conv2d((x_mid * (1 - max_mask2[:, 0]))[:, None], gpu(x_mid_filt).to('cuda:2'), padding=1)
        xm_conv2 = func.conv2d((x_mid * (1 - max_mask1[:, 0]))[:, None], gpu(x_mid_filt).to('cuda:2'), padding=1)

        x_left = (xo + 1) * p
        x_left_filt = flip_filt(np.array([[diag, 0, 0], [1, 0, 0], [diag, 0, 0]], ndmin=4))
        xl_conv1 = func.conv2d((x_left * (1 - max_mask2[:, 0]))[:, None], gpu(x_left_filt).to('cuda:2'), padding=1)
        xl_conv2 = func.conv2d((x_left * (1 - max_mask1[:, 0]))[:, None], gpu(x_left_filt).to('cuda:2'), padding=1)

        x_right = (xo - 1) * p
        x_right_filt = flip_filt(np.array([[0, 0, diag], [0, 0, 1], [0, 0, diag]], ndmin=4))
        xr_conv1 = func.conv2d((x_right * (1 - max_mask2[:, 0]))[:, None], gpu(x_right_filt).to('cuda:2'), padding=1)
        xr_conv2 = func.conv2d((x_right * (1 - max_mask1[:, 0]))[:, None], gpu(x_right_filt).to('cuda:2'), padding=1)

        xo_ps = (xm_conv1 + xl_conv1 + xr_conv1) * mult_1 + (xm_conv2 + xl_conv2 + xr_conv2) * mult_2

        y_mid = yo * p
        y_mid_filt = np.array([[0, 0, 0], [1, 1, 1], [0, 0, 0]], ndmin=4)
        ym_conv1 = func.conv2d((y_mid * (1 - max_mask2[:, 0]))[:, None], gpu(y_mid_filt).to('cuda:2'), padding=1)
        ym_conv2 = func.conv2d((y_mid * (1 - max_mask1[:, 0]))[:, None], gpu(y_mid_filt).to('cuda:2'), padding=1)

        y_up = (yo + 1) * p
        y_up_filt = flip_filt(np.array([[diag, 1, diag], [0, 0, 0], [0, 0, 0]], ndmin=4))
        yu_conv1 = func.conv2d((y_up * (1 - max_mask2[:, 0]))[:, None], gpu(y_up_filt).to('cuda:2'), padding=1)
        yu_conv2 = func.conv2d((y_up * (1 - max_mask1[:, 0]))[:, None], gpu(y_up_filt).to('cuda:2'), padding=1)

        y_down = (yo - 1) * p
        y_down_filt = flip_filt(np.array([[0, 0, 0], [0, 0, 0], [diag, 1, diag]], ndmin=4))
        yd_conv1 = func.conv2d((y_down * (1 - max_mask2[:, 0]))[:, None], gpu(y_down_filt).to('cuda:2'), padding=1)
        yd_conv2 = func.conv2d((y_down * (1 - max_mask1[:, 0]))[:, None], gpu(y_down_filt).to('cuda:2'), padding=1)
...
def inferlist(P, xyzi_est, frame_index, candi_thre, threshold, z_scale, int_scale, x_scale, y_scale, plot_num=0):

    plot_data = {'Probs': [], 'XO': [], 'YO': [], 'ZO': [], 'Int': [], 'BG': [], 'XO_sig': [], 'YO_sig': [],
                 'ZO_sig': [], 'Int_sig': [], 'Sample_ps': []}
    xo = torch.squeeze(xyzi_est[:, 0, :, :])
    yo = torch.squeeze(xyzi_est[:, 1, :, :])
    zo = torch.squeeze(xyzi_est[:, 2, :, :])
    ints = np.squeeze(cpu(xyzi_est[:, 3, :, :]))
    xo_sig = torch.squeeze(xyzi_est[:, 0, :, :])
    yo_sig = torch.squeeze(xyzi_est[:, 1, :, :])
    zo_sig = torch.squeeze(xyzi_est[:, 2, :, :])
    ints_sig = np.squeeze(cpu(xyzi_est[:, 3, :, :]))
    p_nms = torch.squeeze(P)
    p_nms = np.array(nms_func(p_nms, candi_thre=candi_thre))
    xo = np.array(cpu(xo))
    yo = np.array(cpu(yo))
    zo = np.array(cpu(zo))
    ints = np.array(cpu(ints))

    plot_data['Probs'].append(p_nms[plot_num-1])
    plot_data['XO'].append(xo[plot_num-1])
    plot_data['YO'].append(yo[plot_num-1])
    plot_data['ZO'].append(zo[plot_num-1])
    plot_data['Int'].append(ints[plot_num-1])
    plot_data['XO_sig'].append(xo_sig[plot_num-1])
    plot_data['YO_sig'].append(yo_sig[plot_num-1])
    plot_data['ZO_sig'].append(zo_sig[plot_num-1])
    plot_data['Int_sig'].append(ints_sig[plot_num-1])

    sample = np.where(p_nms > threshold, 1, 0)
    plot_data['Sample_ps'].append(np.nonzero(sample[plot_num-1]))
    # number of examples in the batch
    pred_list = []
    for i in range(len(xo)):
        pos = np.nonzero(sample[i])  # get the deterministic pixel position
        for j in range(len(pos[0])):
            pred_list.append([int(frame_index[i])+1,
                              float((0.5 + pos[1][j] + xo[i, pos[0][j], pos[1][j]]) * x_scale),
                              float((0.5 + pos[0][j] + yo[i, pos[0][j], pos[1][j]]) * y_scale),
                              float(zo[i, pos[0][j], pos[1][j]]*z_scale),
                              float(ints[i, pos[0][j], pos[1][j]]*int_scale),
                              float(p_nms[i, pos[0][j], pos[1][j]]),
                              float(xo[i, pos[0][j], pos[1][j]]),
                              float(yo[i, pos[0][j], pos[1][j]])])

    return pred_list, plot_data