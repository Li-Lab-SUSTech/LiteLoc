...
class LocModel:
    def __init__(self, params, device = 'cuda'):
        ...
        if params.Training.infer_data is not None:
            params.Training.bg, params.Training.photon_range, params.Training.factor, params.Training.offset \
                = calculate_bg_factor_offset(params)
        ...
        if params.Training.model_init is not None:
            ...
            self.network.load_state_dict(checkpoint.DECODE.state_dict())
    ...
    def train(self):
        ...
        while self.start_epoch < self.params.Training.max_epoch:
            ...
            for i in range(0, self.params.Training.eval_iteration):
                ...
                p, xyzi_est, xyzi_sig, psf_imgs_est  = self.network.forward(imgs_sim)
    ...
    def evaluation_spline(self):
        ...
        with torch.set_grad_enabled(False):
            for batch_ind, (xemit, yemit, z, S, Nphotons, s_mask, gt, img_sim) in enumerate(
                ...
                P, xyzi_est, xyzi_sig, psf_imgs_est = self.network.forward(img_sim.squeeze())
    ...
    def analyze(self, im):
        ...
        p, xyzi_est, xyzi_sig, _ = self.network.forward(im)
