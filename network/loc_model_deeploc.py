...
from utils.help_utils import calculate_bg, cpu, gpu, calculate_factor_offset, gpu_cpu_torch
from liteloc.loss_utils import LossFuncs_wo_weight_add_bgloss, LossFuncs
from utils.data_generator import DataGenerator
from liteloc.deeploc import DeepLoc
from liteloc_wo_local import LiteLoc_wo_local
...
class DeeplocModel:
    def __init__(self, params, device = 'cuda'):
        ...
        self.DeepLoc = DeepLoc(params.Training.factor, params.Training.offset).to(self.device)

        self.EvalMetric = EvalMetric(params.PSF_model, params.Training)

        self.net_weight = list(self.DeepLoc.parameters())
        ...
        if params.Training.model_init is not None:
            ...
            self.DeepLoc.load_state_dict(checkpoint.DECODE.state_dict())
            self.optimizer.load_state_dict(checkpoint.optimizer.state_dict())
            self.scheduler.last_epoch = self.start_epoch
        else:
            self.start_epoch = 0

        self.criterion = LossFuncs_wo_weight_add_bgloss(train_size=params.Training.train_size[0])
    ...
    def train_spline(self):
        ...
        while self.start_epoch < self.params.Training.max_epoch:
            ...
            for i in range(0, self.params.Training.eval_iteration):
                ...
                p, xyzi_est, xyzi_sig, psf_imgs_est  = self.DeepLoc.forward(imgs_sim)

                loss_total = self.criterion.final_loss(p, xyzi_est, xyzi_sig, xyzi_gt, s_mask, locs, psf_imgs_est, psf_imgs_gt)

                self.optimizer.zero_grad()

                loss_total.backward()
                # avoid too large gradient
                torch.nn.utils.clip_grad_norm_(list(self.DeepLoc.parameters()), max_norm=0.03, norm_type=2)
    ...
    def evaluation_spline(self):
        ...
        with torch.set_grad_enabled(False):
            for batch_ind, (xemit, yemit, z, S, Nphotons, s_mask, gt) in enumerate(
                    self.valid_data):
                img_sim = self.DataGen.simulate_image_eval(s_mask[0], gt[0], S, torch.squeeze(xemit),
                                                      torch.squeeze(yemit), torch.squeeze(z), torch.squeeze(Nphotons))[:, 0]

                P, xyzi_est, xyzi_sig, psf_imgs_est = self.DeepLoc.forward(img_sim)
                gt, s_mask, S = gt[:, 1:-1], s_mask[:, 1:-1], S[:, 1:-1]
                loss_total = self.criterion.final_loss(P, xyzi_est, xyzi_sig, gt, s_mask, S, None, None)
        ...
        with torch.no_grad():
            with autocast():
                for i in range(int(np.ceil(sampling_num*zstack/self.params.Training.batch_size))):
                    ...
                    liteloc_molecule_tensor = self.DeepLoc.analyze(img, threshold=0.98)
    ...
    def train_spline(self):
        ...
        while self.start_epoch < self.params.Training.max_epoch:
            ...
            for i in range(0, self.params.Training.eval_iteration):
                ...
                p, xyzi_est, xyzi_sig = self.LiteLoc.forward(imgs_sim)
    ...
    def evaluation_spline(self):
        ...
        with torch.set_grad_enabled(False):
            for batch_ind, (xemit, yemit, z, S, Nphotons, s_mask, gt) in enumerate(
                ...
                P, xyzi_est, xyzi_sig = self.LiteLoc.forward(img_sim)
                gt, s_mask, S = gt[:, 1:-1], s_mask[:, 1:-1], S[:, 1:-1]
        ...
        for i in tqdm.tqdm(range(sampling_num)):
            ...
            for j in range(zstack):
                ...
                ground_truth[j] = [frame_count,
                                   ...
                                   self.params.PSF_model.vector_psf.pixel_size_xy[1],
                                   zemit[j] + 0, cpu(Nphotons[j])]
            psfs = PSF_torch.simulate_parallel(gpu(xemit), gpu(yemit), zemit.to(self.device), Nphotons)  # xyz's reference is center of image
            psfs = F.pad(psfs, pad=(1, 0, 1, 0), mode='constant', value=0)
            data = psfs + bg[:, None, None]
