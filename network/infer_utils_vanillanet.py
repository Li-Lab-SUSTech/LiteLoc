...
class PsfInfer:
    def __init__(self,infer_par,eval_params, offset=0, factor=0):
        ...
        self.model = vanillanet_dilated_6().cuda()
        # self.deploy(infer_par['net_path'])
        # self.model = vanillanet_7_v1(deploy=True)
        self.model.cuda()
    ...
    def deploy(self, path):
        ...
        model_ckpt = dict()
        model_ckpt['state_dict'] = self.model.state_dict()
        model_ckpt['epoch'] = checkpoint['epoch']
        model_ckpt['optimizer'] = checkpoint['optimizer']
        torch.save(model_ckpt, 'vanillanet_deploy.pth.tar')
    ...
    def inferdata(self):
            ...
            with torch.set_grad_enabled(False):
                with autocast():
                    for ind, (index, coord, img) in enumerate(self.infer_loader, start=self.start_ind):
                        ...
                        time_forward = (time.time() - time_forward) * 1000  # time for network forward pass (per frame, ms)
        ...
        print("predict finish, \n time for network forward pass(per frame) is " + str(time_forward) + "ms, \n" + "time for post-process(per frame) is "
